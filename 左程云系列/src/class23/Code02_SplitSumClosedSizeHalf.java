...
public class Code02_SplitSumClosedSizeHalf {
	...
	private static int process(int[] arr, int index, int k, int res) {
		if(res < 0){
		...
		int p1 = process(arr,index+1,k,res);

		int p2 = arr[index] + process(arr,index+1,k-1,res - arr[index]);
	...
	public static int dp2(int[] arr){
		...
		int[][][] dp = new int[arr.length+1][arr.length+1][sum/2+1];
		...
		for(int index = arr.length-1; index>=0; index--){
			for(int k=1; k<=arr.length; k++){
				for(int res=0; res<=sum/2; res++){
					int p1 = dp[index+1][k][res];

					int p2 = -1;
					if(arr[index] <= res){
						int i = dp[index + 1][k - 1][res - arr[index]];
						if(i != -1){
							p2 = arr[index] + i;
						}

					}
					dp[index][k][res] = Math.max(p1,p2);
				}
			}
		}
		if(arr.length % 2 == 0){
